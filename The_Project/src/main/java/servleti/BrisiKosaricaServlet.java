...
@WebServlet(name = "BrisiKosaricaServlet", urlPatterns = {"/BrisiKosaricaServlet"})
public class BrisiKosaricaServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            for ( ProizvodUKosarici puk: kosarica) {
                if (puk.getIDProizvod()==id) { kosarica.remove(puk); break; }
            }
            request.getSession().setAttribute("kosarica",kosarica);
            
        }
        catch(NumberFormatException e){}
        response.sendRedirect("PrikazKosariceServlet");        
    }
